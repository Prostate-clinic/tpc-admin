...
export default function MyPostsPage() {
  ...
  async function load(pageNum: number = 1) {
    ...
    setError("");
    try {
      const data = await nestApi.getMyBlogs(pageNum, 12);
      setBlogs(data.blogs ?? []);
      setPagination(data.pagination);
      setPage(pageNum);
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : "Failed to load your posts");
    } finally {
      setLoading(false);
    }
  }
        ...
        <div className="space-y-4">
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {blogs.map((blog) => {
              ...
              return (
                <div key={blog.id} className="smooth-card p-2 border-1 border-slate-200 rounded-lg flex flex-col gap-3">
                  {blog.coverImage && (
                    <img
                      src={blog.coverImage}
                      alt={blog.title}
                  ...
                  <div className="mt-auto border-t border-slate-100 pt-3 text-right text-xs text-slate-400">
              ...
              <div className="flex items-center gap-2">
                {Array.from({ length: pagination.totalPages }, (_, i) => i + 1).map((p) => (
                  <button
                    ...
                    onClick={() => load(p)}
                    disabled={loading}
                    className={`rounded-lg px-3 py-2 text-sm font-medium transition ${page === p
